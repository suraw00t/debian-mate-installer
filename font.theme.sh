...
function _omb_theme_PROMPT_COMMAND() {
    ...
    local hostname="${_omb_prompt_bold_red}\u@\h"
    local python_venv; _omb_prompt_get_python_venv
    python_venv=$_omb_prompt_white$python_venv

    # Set return status color
    if [[ ${RC} == 0 ]]; then
        ret_status="${_omb_prompt_bold_teal}"
    else
        ret_status="${_omb_prompt_bold_brown}${RC}${_omb_prompt_bold_teal} "
    fi

    # Append new history lines to history file
    history -a

#    PS1="$(clock_prompt)$python_venv${hostname}${_omb_prompt_bold_white}: ${_omb_prompt_bold_green}[\[\033[01;34m\]\w\[\033[00m\]] $(scm_prompt_char_info)
    PS1="$(clock_prompt)$python_venv${hostname}${_omb_prompt_bold_navy}: ${_omb_prompt_bold_green}[${_omb_prompt_bold_white}\w${_omb_prompt_bold_green}] $(scm_prompt_char_info)
